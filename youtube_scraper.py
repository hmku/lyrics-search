...
def _search_results(query, number_results, language):
    assert isinstance(query, str), 'Search term must be a string'
    assert isinstance(number_results, int), 'Number of results must be an integer'
    replaced_search_term = query.replace(' ', '+')
    google_url = 'https://www.google.com/search?q={}&num={}&hl={}&as_sitesearch=youtube.com'.format(replaced_search_term, number_results, language)
...
def _scrape_google(search_term, number_results, language_code):
    try:
